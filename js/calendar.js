async function initGcal() {
  ...
  try {
    ...
    gcalOk = false;
    gcalState("err", "Google Calendar unavailable");
  }
}
...
async function syncAllGcal() {
  ...
  for (const item of D.activities) {
    if (item.status === "booked" && item.startDate && !item.gcalEventId) {
  ...
  for (const item of D.accommodation) {
    if (item.status === "booked" && item.checkin && !item.gcalEventId) {
      item.gcalEventId = await mkGcalEvent(
        "🏨 " + item.name,
        item.notes || "",
        item.checkin,
...
async function forceSyncAllGcal() {
  if (!gcalOk) {
    toast("Google Calendar not connected", "error");
    return;
  }
  const btn = $("btn-sync-gcal");
  btn.disabled = true;
  ...
  for (const item of D.activities) {
    if (item.status === "booked" && item.startDate && !item.gcalEventId) {
      // ← added !item.gcalEventId
      const eid = await mkGcalEvent(
        "🎯 " + item.name,
        item.notes || "",
        item.startDate,
        item.endDate || null,
  ...
  renderCalendar();
  btn.disabled = false;
  btn.textContent = "📅 Sync to my Calendar";
  toast(
    added > 0
      ? `${added} event${added !== 1 ? "s" : ""} added to your Calendar!`
      : "All booked items already synced ✓",
  );
}
...
async function mkGcalEvent(summary, description, start, end, allDay) {
  try {
    const s = new Date(start),
      e = end ? new Date(end) : new Date(s.getTime() + 3600000);
    const body = allDay
      ? {
          summary,
          description,
          start: { date: start.slice(0, 10) },
          end: { date: (end || start).slice(0, 10) },
        }
      : {
          summary,
          description,
...
function renderCalendar() {
  ...
  D.activities.forEach((a) => {
    if (a.status === "booked" && a.startDate)
      events.push({
        type: "act",
        lbl: a.name,
        ...
        e: a.endDate
          ? new Date(a.endDate)
          : new Date(new Date(a.startDate).getTime() + 3600000),
      });
  });
  D.transport.forEach((t) => {
    if (t.status === "booked" && t.startDate)
      events.push({
        type: "trp",
        lbl: `${TTE[t.type] || "✈️"} ${t.name}`,
        s: new Date(t.startDate),
        e: t.endDate
          ? new Date(t.endDate)
          : new Date(new Date(t.startDate).getTime() + 3600000),
      });
  });
  D.accommodation.forEach((a) => {
    if (a.status === "booked" && a.checkin)
      events.push({
        type: "acc",
        lbl: a.name,
        s: new Date(a.checkin),
        e: a.checkout
          ? new Date(a.checkout)
          : new Date(new Date(a.checkin).getTime() + 86400000),
      });
  });
  const DAYS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  let html = DAYS.map((d) => `<div class="cal-day-hdr">${d}</div>`).join("");
  for (let i = 0; i < firstDay; i++)
    html += `<div class="cal-day other-month"><span class="cal-dn"></span></div>`;
  for (let d = 1; d <= daysInMonth; d++) {
    const date = new Date(y, m, d);
    const isToday = date.toDateString() === today.toDateString();
    const dayEvs = events.filter((ev) => {
      const s = new Date(ev.s);
      s.setHours(0, 0, 0, 0);
      const en = new Date(ev.e);
...
function calClick(y, m, d) {
  ...
  $("cal-add-title").textContent =
    `Add for ${calAddDate.toLocaleDateString([], { month: "long", day: "numeric" })}`;
...
function setCalAddType(type) {
  ...
  $("cal-add-tabs").innerHTML = types
    .map(
      (t, i) =>
        `<button class="btn ${t === type ? "btn-primary" : "btn-secondary"} btn-sm" onclick="setCalAddType('${t}')">${labels[i]}</button>`,
    )
    .join("");
  const ds = calAddDate ? calAddDate.toISOString().slice(0, 10) : "";
  const def9 = `${ds}T09:00`,
    def10 = `${ds}T10:00`;
  const forms = {
    activity: `
...
async function saveCalItem() {
  const name = $("caf-name")?.value.trim();
  if (!name) {
  ...
  if (calAddType === "activity") {
    const item = {
      ...
      status: "booked",
      startDate: start,
      endDate: $("caf-end")?.value || "",
      notes: $("caf-notes")?.value || "",
      mapsLink: "",
      files: [],
      gcalEventId: null,
      createdAt: Date.now(),
    ...
    const item = {
      id: uid(),
      name,
      location: $("caf-loc")?.value || "",
      status: "booked",
      checkin: start,
      checkout: $("caf-end")?.value || "",
      mapsLink: "",
      notes: "",
      files: [],
      gcalEventId: null,
