function renderChecklists() {
  ...
  g.innerHTML = D.checklists
    .map((cl) => {
      const total = cl.items?.length || 0,
        done = cl.items?.filter((i) => i.checked).length || 0;
      const pct = total > 0 ? Math.round((done / total) * 100) : 0;
      const trip = cl.tripId
        ? D.transport.find((t) => t.id === cl.tripId)
        : null;
...
$("btn-save-checklist").addEventListener("click", () => {
  const name = getVal("cl-name").trim();
  if (!name) {
...
function addCheckItem(id) {
  ...
  if (!text) return;
  const cl = D.checklists.find((x) => x.id === id);
  if (!cl.items) cl.items = [];
  cl.items.push({ id: uid(), text, checked: false });
  inp.value = "";
  renderChecklists();
  saveDrive();
}
...
function delChecklist(id) {
  ...
  D.checklists = D.checklists.filter((x) => x.id !== id);
  renderChecklists();
  saveDrive();
  toast("Deleted");
}
