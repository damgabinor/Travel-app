async function loadDrive() {
  ...
  try {
    ...
    if (driveFileId) {
      const c = await gapi.client.drive.files.get({
        fileId: driveFileId,
        alt: "media",
      });
      const loaded = JSON.parse(c.body);
      // migrate legacy keys
      if (loaded.trips && !loaded.transport) {
        loaded.transport = loaded.trips;
        delete loaded.trips;
      }
      if (loaded.activities) {
        const wishes = loaded.activities.filter(
          (a) => a.itemType === "wishlist",
        );
        const acts = loaded.activities.filter(
          (a) => a.itemType === "activity" || !a.itemType,
        );
        if (!loaded.wishlist || !loaded.wishlist.length)
          loaded.wishlist = wishes;
        loaded.activities = acts.map((a) => {
          delete a.itemType;
          return a;
        });
      }
...
async function saveDrive() {
  ...
  try {
    if (driveFileId) {
      ...
      const m = await gapi.client.drive.files.create({
        resource: { name: DATA_FILE, mimeType: "application/json" },
        fields: "id",
      });
